...
const socketGenerator = (observable = observableGenerator()) => ({
...
describe('Controller', () => {
  ...
  beforeEach(() => {
    Observable.mockClear();
    Observable.from = jest.fn(observableGenerator);
    socketServer = {};
  ...
  it('should call socket subscribe on disconnect observable', () => {
    ...
    const socket = socketGenerator(observable);
    socket.disconnect.mockReturnValueOnce(observable);
    controller.handleSource(socket);
  ...
  it('should call let on the "bridge-data" Observable', () => {
    ...
    const socket = socketGenerator(observable);
    controller.handleSource(socket);
    expect(observable.let).toBeCalledWith(middlewareContainer.handle);
  });
  ...
  it('should call map on the "bridge-data" Observable', () => {
    ...
    const socket = socketGenerator(observable);
    controller.handleSource(socket);
    expect(observable.map).toBeCalledWith(controller.moduleMapper);
  });
  ...
  it('should call subscribe on the "bridge-data" Observable', () => {
    ...
    const socket = socketGenerator(observable);
    controller.handleSource(socket);
    expect(observable.subscribe).toBeCalledWith(emitterContainer.handle);
  });
  ...
  it('should call map with emitterContainer.get in handle client', () => {
    const socket = socketGenerator();
    const observable = observableGenerator();
    Observable.from.mockReturnValueOnce(observable);
    controller.handleClient(socket);
    expect(observable.map).toBeCalledWith(emitterContainer.get);
  });

  it('should handleClient call the subscription method when disconnected', () => {
    const socket = socketGenerator();
    socket.id = 'idNumberString';
    const observable = observableGenerator();
    socket.disconnect.mockReturnValueOnce(observable);
    controller.handleClient(socket);
    const func = observable.subscribe.mock.calls[0][0];
    console.log = jest.fn();
    func();
    expect(console.log).toBeCalledWith('Client - idNumberString: disconnected');
  });
  ...
  it('should handleSubscription call obs.do', () => {
    const obs = observableGenerator();
    const socket = socketGenerator(obs);
    const emitter = { type: 'hello' };
    Controller.handleSubscription(socket)(emitter);
    expect(obs.do).toBeCalledWith(expect.any(Function));
  });
});
